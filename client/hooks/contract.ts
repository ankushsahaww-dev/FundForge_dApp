...
// ============================================================
// Crowdfunding dApp — Contract Methods
// ============================================================
 ...
 * Calls: create_campaign(creator: Address, title: String, description: String, goal: i128, deadline: u64) -> u64
 */
 ...
 * Calls: donate(donor: Address, campaign_id: u64, amount: i128)
 */
export async function donate(
  ...
  return callContract(
    "donate",
    [toScValAddress(caller), toScValU64(campaignId), toScValI128(amount)],
    caller,
    true
  );
}
 ...
 * Calls: withdraw(creator: Address, campaign_id: u64)
 */
export async function withdraw(
  ...
  return callContract(
    "withdraw",
    [toScValAddress(caller), toScValU64(campaignId)],
    caller,
    true
  );
}
...
/**
 * Claim refund from a failed campaign (donor only).
 * Calls: claim_refund(donor: Address, campaign_id: u64)
 */
 ...
 * Calls: get_campaign(campaign_id: u64) -> Campaign
 */
...
/**
 * Get donation amount for a specific donor (read-only).
 * Calls: get_donation(campaign_id: u64, donor: Address) -> i128
 */
export async function getDonation(
  ...
  return readContract(
    "get_donation",
    [toScValU64(campaignId), toScValAddress(donor)],
    caller
  );
}

/**
 * Get all campaign IDs (read-only).
 * Calls: get_all_campaign_ids() -> Vec<u64>
 */
export async function getAllCampaignIds(caller?: string) {
  return readContract("get_all_campaign_ids", [], caller);
}
